...
class MqttReceiverNode(Node):
    def __init__(self, config: BridgeConfig):
        ...
        self.get_logger().info(
            f"Starting MQTT connection thread to {config.broker}:{config.port}"
        )
    ...
    def _connect_loop(self):
        while not self._shutdown_event.is_set():
            try:
                self.get_logger().info(
                    f"Attempting to connect to MQTT broker {self._config.broker}:{self._config.port}..."
                )
                self._mqtt_client.connect(self._config.broker, self._config.port)
                self._mqtt_client.loop_start()
                self.get_logger().info("MQTT connection established and loop started.")
                break
            except Exception as e:
                self.get_logger().warn(
                    f"MQTT connection failed: {e}. Retrying in 5 seconds..."
                )
                for _ in range(50):
